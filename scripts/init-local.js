...
runScript(async (token, ownerAddress, web3, watcher) => {
  ...
  const worker1Token = new ExpandToken(web3.currentProvider, accounts.worker1.address);
  await worker1Token.init();

  const worker2Token = new ExpandToken(web3.currentProvider, accounts.worker2.address);
  await worker2Token.init();

  const requester1Token = new ExpandToken(web3.currentProvider, accounts.requester1.address);
  await requester1Token.init();

  const requester2Token = new ExpandToken(web3.currentProvider, accounts.requester2.address);
